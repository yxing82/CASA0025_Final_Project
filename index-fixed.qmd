...
<div class="contributors-grid">
  <a href="https://github.com/yxing82" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">YX</div>
    <span class="contributor-name">Yujing (Olivia) Xing</span>
    <span class="contributor-role">GEE App Architecture, UI/UX Design & Development, Concept Direction</span>
    <span class="contributor-github">github.com/yxing82</span>
  </a>
  <a href="https://github.com/lukegbenson" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">LB</div>
    <span class="contributor-name">Luke Benson</span>
    <span class="contributor-role">SMA/FVC Methodology, Vegetation Analysis, Project Concept</span>
    <span class="contributor-github">github.com/lukegbenson</span>
  </a>
  <a href="https://github.com/casagochman" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">YG</div>
    <span class="contributor-name">Yoav Gochman</span>
    <span class="contributor-role">LST Analysis, Water Consumption, Preprocessing</span>
    <span class="contributor-github">github.com/casagochman</span>
  </a>
  <a href="https://github.com/christychoicc" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">CC</div>
    <span class="contributor-name">Christy Choi</span>
    <span class="contributor-role">Preprocessing, GEE UI/UX Design, Legend System, Documentation</span>
    <span class="contributor-github">github.com/christychoicc</span>
  </a>
  <a href="https://github.com/hawaiie-star" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">WT</div>
    <span class="contributor-name">Waiie Tsang</span>
    <span class="contributor-role">Literature Review, Narrative Content</span>
    <span class="contributor-github">github.com/hawaiie-star</span>
  </a>
  <a href="https://github.com/Nanxi-lu" class="contributor-card">
    <div class="contributor-avatar" style="background:#7a8152;">NL</div>
    <span class="contributor-name">Nanxi Lu</span>
    <span class="contributor-role">Literature Review, Narrative Content</span>
    <span class="contributor-github">github.com/Nanxi-lu</span>
  </a>
</div>
```
...
<style>
/* ── Scroll offset ──────────────────────────────────────────────── */
/* The navbar/banner is position:relative and scrolls away, so we
   only need a tiny breathing gap, NOT a big fixed-header offset.
   The real scroll-margin-top rules are in styles.css — this is
   just a safety duplicate in case styles.css loads late.          */
html {
  scroll-behavior: smooth;
  scroll-padding-top: 1rem;
}
h1, h2, h3, h4, section[id] {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ── STEP 1: FIX QUARTO'S ID PLACEMENT ───────────────────────────
  // Quarto puts id on <section>, not on <h2>.
  // We move each section's id onto its first child heading,
  // so anchor scrolling targets the heading directly.
  ...
  // ── STEP 2: GRAB REFERENCES ─────────────────────────────────────
  var dropdown = document.querySelector(".contents-dropdown");
  var button   = document.querySelector(".contents-button");
  var links    = document.querySelectorAll(".contents-menu a[href^='#']");
  var appZone  = document.querySelector(".app-embed-zone") ||
                 document.querySelector(".column-page");
  if (!dropdown || !button) return;

  // ── STEP 3: CALCULATE OFFSET ────────────────────────────────────
  // Navbar is position:relative (scrolls away), so we just need
  // a small breathing gap at the top of the viewport.
  var OFFSET = 16; // px — just a small gap

  // ── STEP 4: CLICK HANDLER — scroll heading to top ───────────────
  links.forEach(function (link) {
    link.addEventListener("click", function (e) {
      ...
      // Safety: if the id is still on a <section>, find heading inside
      var scrollTarget = target;
      if (target.tagName === "SECTION" || target.tagName === "DIV") {
        scrollTarget = target.querySelector("h1, h2, h3, h4") || target;
      }

      var targetTop = scrollTarget.getBoundingClientRect().top + window.pageYOffset;
      window.scrollTo({
  ...
  // ── STEP 6: HIDE DROPDOWN OVER APP EMBED ────────────────────────
  function updateContentsVisibility() {
    if (!appZone) return;
    var dropdownRect = dropdown.getBoundingClientRect();
    var appRect = appZone.getBoundingClientRect();
    var overlaps =
  ...
  // ── STEP 7: HANDLE DIRECT URL HASH ON PAGE LOAD ─────────────────
  if (window.location.hash) {
    setTimeout(function () {
      ...
      var scrollTarget = hashTarget;
      if (hashTarget.tagName === "SECTION" || hashTarget.tagName === "DIV") {
        scrollTarget = hashTarget.querySelector("h1, h2, h3, h4") || hashTarget;
      }
