...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function scrollToSection(id) {
    ...
    /*
      This controls where the heading appears.
      0.22 means the heading lands about 22% down from the top of the screen.
      This shows the heading clearly and leaves the majority of the section visible.
    */
    const offset = Math.round(window.innerHeight * 0.22);
    const targetTop = target.getBoundingClientRect().top + window.pageYOffset;
    const position = Math.max(targetTop - offset, 0);
  ...
  links.forEach(function (link) {
    link.addEventListener("click", function (event) {
      ...
      /*
        Remove the hash so the browser/Quarto does NOT do its own jump.
        Then we control the scroll position ourselves.
      */
