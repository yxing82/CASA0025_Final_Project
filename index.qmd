...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function scrollToSection(id) {
    ...
    /*
      LANDING POSITION:
      160px means the section heading appears near the top-middle area,
      with the section content visible underneath.
      Smaller number = heading closer to top.
      Bigger number = heading lower down.
    */
    const landingFromTop = 160;

    const targetTop = target.getBoundingClientRect().top + window.pageYOffset;
    const position = Math.max(targetTop - landingFromTop, 0);
