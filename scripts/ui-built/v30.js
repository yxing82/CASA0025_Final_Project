// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v30
// add all pre-calculated baseline bands: LST_diff_2019 & et_diff_2019
// ═══════════════════════════════════════════════════════════════════════
...
var state = {
  currentView:   'explore', activeLayer:   'greenspace', timeMode:      'compare',
...
function updateLegendVisibility() {
  ...
  var isDiff = !isSameDate && !actuallySplit && state.timeMode !== 'baseline2019'; // Unified custom diff view

  var isTemp = state.activeLayer === 'temperature';
  var isTempDiff = isTemp && (isDiff || (state.timeMode === 'baseline2019' && !actuallySplit));
  ...
  var isWaterDiff = isWater && (isDiff || (state.timeMode === 'baseline2019' && !actuallySplit));
...
function updateTimeModeUI() {
  ...
  var hasBaselineOpt = timeModeSel.items().length() === 3;
  ...
  if (config.hasBaseline2019 && !hasBaselineOpt) {
    ...
    if (state.timeMode === 'baseline2019') { state.timeMode = 'single'; timeModeSel.setValue('Single month'); }
  }
  ...
  if (state.timeMode === 'single') {
    ...
    toLabel.setValue('Target'); fromRow.style().set('shown', false); toRow.style().set('shown', true);
  }
}
...
function updateLayerPills() { [pillGreen, pillTemp, pillWater, pillSatellite].forEach(function(p) { stylePillButton(p, p._key === state.activeLayer); }); }
...
function onTimeChange() {
  if (state.timeMode === 'single') {
    ...
    state.toMonth = MONTHS.indexOf(toMonthSel.getValue()) + 1; state.toYear  = parseInt(toYearSel.getValue(), 10); state.fromMonth = state.toMonth; state.fromYear = 2019; 
  } else {
...
timeModeSel.onChange(function(val) {
  if (val === 'Single month') state.timeMode = 'single'; else if (val === 'Compare two months') state.timeMode = 'compare'; else if (val === 'Compare to 2019') state.timeMode = 'baseline2019';
...
var controlsPanel = ui.Panel([makeSectionTitle('Map controls'), ui.Label('Layer', STYLES.muted), ui.Panel([pillGreen, pillTemp, pillWater, pillSatellite], ui.Panel.Layout.flow('horizontal')), makeSeparator(), ui.Label('Analysis mode', STYLES.muted), timeModeSel, fromRow, toRow, makeSeparator(), splitCheck], null, STYLES.section);
...
var statsPanel = ui.Panel([statsTitleLabel, statsGrid1, statsGrid2, statsGrid3], null, STYLES.section); statsPanel.style().set('shown', false);

var trendPanel = ui.Panel([], null, STYLES.section); 

var comparePromptPanel = ui.Panel([makeSectionTitle('Compare areas'), ui.Label('Click a census tract directly on the map to select your first area for comparison.', {fontSize: '12px', color: '#' + COLORS.gray, padding: '12px 0'})], null, STYLES.section); comparePromptPanel.style().set('shown', false);
var compareContent = ui.Panel([], null, {stretch: 'horizontal'}); compareContent.style().set('shown', false);

var aboutBody = {fontSize: '12px', color: '#' + COLORS.black, whiteSpace: 'pre-wrap', padding: '2px 0 6px 0'};
var aboutContent = ui.Panel([makeSectionTitle('About this dashboard'), ui.Label('This dashboard tracks the removal of non-functional turf across the Las Vegas Valley...', aboutBody), makeSeparator(), ui.Label('Policy background', STYLES.subtitle), ui.Label('Nevada Assembly Bill 356 (2021) requires the removal of all non-functional turf...', aboutBody), ui.Label('The Southern Nevada Water Authority (SNWA) oversees compliance...', aboutBody)], null, STYLES.section); aboutContent.style().set('shown', false);
...
function setActiveTab(tab) {
  ...
  tractHeaderPanel.style().set('shown', tab === 'explore' && state.selectedTract !== null); promptPanel.style().set('shown', tab === 'explore' && state.selectedTract === null); comparePromptPanel.style().set('shown', tab === 'compare');
  if (tab === 'explore') refreshMap(); if (tab === 'compare') updateCompareView(); updateHighlightLayer();
}

tabExplore.onClick(function() { setActiveTab('explore'); }); tabCompare.onClick(function() { setActiveTab('compare'); }); tabAbout.onClick(function() { setActiveTab('about'); });
...
function updateSidePanel(tract) {
  ...
  ee.Dictionary({
    ...
    if (state.timeMode === 'baseline2019') {
      statsTitleLabel.setValue('Comparison to 2019'); statsGrid2.style().set('shown', false); statsGrid3.style().set('shown', false);
      var fvcChangePct = fS > 0 ? ((fE - fS) / fS * 100) : 0;
      cardGreen.name.setValue('Green space change'); cardGreen.val.setValue(fvcChangePct.toFixed(1) + '%'); cardGreen.delta.setValue('compared to same month in 2019'); cardGreen.delta.style().set('color', fvcChangePct < 0 ? COLORS.red : COLORS.green);
      trendPanel.widgets().reset(buildBaselineTrendCharts(tractGeom, state.toYear));
    } else if (isSameDate) {
...
function updateCompareView() {
  ...
  if (state.timeMode === 'baseline2019') {
      ...
      ee.Dictionary({
        ...
        var charts = buildBaselineCompareTrendCharts(tractA, tractB, state.toYear);

        compareContent.widgets().reset([
          ui.Label('Differences compared to the same month in 2019', {fontSize: '11px', color: COLORS.gray, padding: '0 0 4px 0'}),
