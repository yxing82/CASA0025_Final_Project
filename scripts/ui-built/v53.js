// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v53
// update satellite layer - static highlights for satellite split view
// ═══════════════════════════════════════════════════════════════════════
...
// PERFORMANCE: FeatureView uses pre-computed tiles — no Collection.style() per tile.
// Correct visParams use polygon-prefixed properties (polygonStrokeColor, etc.).
// Separate L/R instances required; a widget can only belong to one map.
...
function enforceZoomLimit(currentZoom) {
  if (currentZoom < MIN_ZOOM) {
    // only need to set the left map; the ui.Map.Linker handles the right map automatically
...
// }
// update in v50
// lock highlight only for each tab
function updateHighlightLayer() {
  [leftMap, rightMap].forEach(function(m) {
    var layers = m.layers(); var layersToRemove = [];
  ...
  if (state.currentView === 'compare' && state.compareTracts.length > 0) {
    ...
    leftMap.addLayer(compOverlay, {}, 'Compare tracts'); if (containerMode === 'split') rightMap.addLayer(compOverlay, {}, 'Compare tracts');
  }

  if (state.currentView === 'explore' && state.selectedTract) {
    var highlightStyle = ee.FeatureCollection([ee.Feature(state.selectedTract.geometry())]).style({color: COLORS.primary, fillColor: COLORS.primary + '30', width: 2});
    leftMap.addLayer(highlightStyle, {}, 'Selected tract'); if (containerMode === 'split') rightMap.addLayer(highlightStyle, {}, 'Selected tract');
  }
}
...
// Insert data layer at index 0 (bottom of stack).
function _addData(map, image, vis, name) {
  map.layers().insert(0, ui.Map.Layer(image, vis, name, true));
}

// Remove and re-add FeatureView overlays at the end of each refresh so they sit
// on top of data imagery. Boundary first, then tracts (tracts render on top).
...
// update in v51 - split view unchanged for satellite layer
function refreshMap() {
  var isSameDate = (state.fromYear === state.toYear && state.fromMonth === state.toMonth);
  var isBaseline = (state.dataType === 'adjusted');
  ...
  if (isSatellite) {
    ...
    actuallySplit = state.splitEnabled && (!isSameDate || isBaseline);
  }
