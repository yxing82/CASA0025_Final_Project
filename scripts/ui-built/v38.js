// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v38
// fixed the zoom-in and zoom-out issues when change the "split view" toggle
// ═══════════════════════════════════════════════════════════════════════
...
function refreshMap() {
  ...
  if (actuallySplit) {
    ...
    if (isSameDate && !isBaseline) {
      if (state.activeLayer === 'greenspace') { _addData(leftMap, getFVCImage(state.fromYear, state.fromMonth), fvc_vis, 'FVC'); _addData(rightMap, getFVCImage(state.fromYear, state.fromMonth), fvc_vis, 'FVC'); }
      else if (state.activeLayer === 'temperature') { _addData(leftMap, getLSTImage(state.fromYear, state.fromMonth), lst_vis, 'LST'); _addData(rightMap, getLSTImage(state.fromYear, state.fromMonth), lst_vis, 'LST'); }
      else if (state.activeLayer === 'water') { _addData(leftMap, getETImage(state.fromYear, state.fromMonth), et_vis, 'Water'); _addData(rightMap, getETImage(state.fromYear, state.fromMonth), et_vis, 'Water'); }
      else { _addData(leftMap, getS2Composite(state.fromYear, state.fromMonth), s2_vis, 'Satellite'); _addData(rightMap, getS2Composite(state.fromYear, state.fromMonth), s2_vis, 'Satellite'); }
    } else {
      if (state.activeLayer === 'greenspace') {
        ...
        _addData(leftMap, fvcDiff, fvc_diff_vis, 'FVC change'); _addData(rightMap, fvcDiff, fvc_diff_vis, 'FVC change');
        ...
        _addData(leftMap, lstDiff, lst_diff_vis, 'LST change'); _addData(rightMap, lstDiff, lst_diff_vis, 'LST change');
        ...
        _addData(leftMap, etDiff, et_diff_vis, 'Water change'); _addData(rightMap, etDiff, et_diff_vis, 'Water change');
      } else { _addData(leftMap, getS2Composite(state.toYear, state.toMonth), s2_vis, 'Current view'); _addData(rightMap, getS2Composite(state.toYear, state.toMonth), s2_vis, 'Current view'); }
    }
  }
...
var refreshMap = function() { _origRefreshMap(); writeUrlState(); };
