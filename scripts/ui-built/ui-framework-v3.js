// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v3 
...
);  // need to match on our own asset ID!!!!
...
// ─── 5.2 & 5.3 Layer and Combine toolbar ──────────────────────────────────────────────
...
// Assemble everything into ONE row
var mainToolbar = ui.Panel([
  // 1. Layer buttons
  pillGreen, pillTemp, pillWater, 
  // 2. A subtle vertical divider
  ui.Label('|', {margin: '4px 16px', color: '#CCCCCC'}), 
  // 3. Time selectors
  ui.Label('From', {fontSize: '11px', color: COLORS.gray, margin: '6px 4px'}),
  fromMonthSel, fromYearSel,
  ui.Label('—', {fontSize: '14px', color: COLORS.gray, margin: '4px 8px'}),
  toMonthSel, toYearSel,
  // 4. Spacer pushes the split view to the far right
  ui.Label('', {stretch: 'horizontal'}), 
  // 5. Split view toggle
  splitLabel, splitCheck
], ui.Panel.Layout.flow('horizontal'), {backgroundColor: COLORS.white, padding: '6px 12px'});
...
function setActiveTab(tab) {
  ...
  // show merged bar from created `mainToolbar`
  mainToolbar.style().set('shown',    tab === 'explore');
