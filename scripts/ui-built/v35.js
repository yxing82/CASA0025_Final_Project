// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v35
// add search name function
// ═══════════════════════════════════════════════════════════════════════
...
// Named places (cities + CDPs) within the study area (v35)
var namedPlaces = ee.FeatureCollection('TIGER/2020/PLACE')
  .filter(ee.Filter.eq('STATEFP', '32'))
  .filterBounds(roi);
...
// ─── PLACE SEARCH / QUICK NAVIGATION (v35) ────────────────────────────────

// 1. Create the select widget once and keep a reference to it
...
// 2. Fetch data: Added .distinct() to prevent duplicate crashes
namedPlaces.aggregate_array('NAME').distinct().sort().evaluate(function(names, error) {
  
  // Catch any background errors and print them to the console
  if (error) {
    print('Error loading places:', error);
    placeSelect.setPlaceholder('Error loading places');
    return;
  }

  // Check if the area actually has places
  if (!names || names.length === 0) {
    print('Warning: No named places found in this boundary.');
    placeSelect.setPlaceholder('No places available');
    return;
  }
  
  // If successful, update the widget and unlock it
  placeSelect.items().reset(names);
  placeSelect.setPlaceholder('Jump to a place...');
  placeSelect.setDisabled(false);
  
  // Set the change callback
  placeSelect.onChange(function(name) {
    var selected = namedPlaces.filter(ee.Filter.eq('NAME', name));
    leftMap.centerObject(selected, 13);
  });
});
...
function setActiveTab(tab) {
  ...
  navPanel.style().set('shown', tab !== 'about');  // v35
