// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v11
// ═══════════════════════════════════════════════════════════════════════
...
var trendPanel = ui.Panel([
  ui.Label('Trend over time', STYLES.subtitle),
  ui.Label(
    'Select a tract to load monthly FVC and LST charts.',
...
var futureAnalysisPanel = ui.Panel([
  ...
  ui.Label('Reserved for later teammate outputs: precomputed monthly stats, trend charts, comparison tables, and modeled summaries.', {fontSize: '11px', color: COLORS.gray, whiteSpace: 'pre-wrap'})
], null, STYLES.section);
...
var statsPanel = ui.Panel([
  ...
  statsGrid2,
  plainEnglish,
  resetSelectionBtn, // addCompareBtn removed
  techToggle,
  techContent
], null, STYLES.section);
...
//   // Limit changed to < 2
//   if (state.selectedTract && state.compareTracts.length < 2) {
...
//           // Color array reduced to just two colors
//           color: [COLORS.blue, COLORS.coral][state.compareTracts.length]
//         });
//         // Label updated to show out of 2
//         addCompareBtn.setLabel('Added (' + state.compareTracts.length + '/2)');
        
//         // updateHighlightLayer();
//         updateHighlightLayer();
//         if (state.currentView === 'compare') updateCompareView();
//         if (state.currentView === 'compare' && state.compareTracts.length === 2) updateCompareView();
//       }
//     });
//   }
// });
...
//   ui.Label(
//     'This dashboard tracks the removal of non-functional turf across the Las Vegas Valley using satellite imagery. ' +
//     'It shows where green space has changed, how land surface temperature has responded, and how much water may have been saved.',
//     aboutBody
//   ),
...
//   ui.Label('Policy background', STYLES.subtitle),
//   ui.Label(
//     'Nevada Assembly Bill 356 (2021) requires the removal of all non-functional turf irrigated with Colorado River water ' +
//     'in the Las Vegas Valley by the end of 2026. Non-functional turf refers to ornamental grass in medians, roundabouts, ' +
//     'commercial streetscapes, and similar areas that serve no recreational purpose.',
//     aboutBody
//   ),
//   ui.Label(
//     'The Southern Nevada Water Authority (SNWA) oversees compliance and provides rebates for turf conversion. ' +
//     'The legislation is part of a broader effort to reduce Colorado River water consumption amid a long-term drought in the region.',
//     aboutBody
//   ),
...
//   ui.Label('Surface temperature', {fontSize: '11px', fontWeight: 'bold', color: '#' + COLORS.orange, margin: '4px 0 0 0'}),
//   ui.Label(
//     'Landsat 8 C2 Level 2 imagery\n  → cloud masking (QA_PIXEL)\n  → scale factors applied\n  → Kelvin to Celsius\n  → monthly median composite\n  → mean per census tract',
//     aboutPipeline
//   ),

//   ui.Label('Water savings estimate', {fontSize: '11px', fontWeight: 'bold', color: '#' + COLORS.blue, margin: '4px 0 0 0'}),
//   ui.Label(
//     'Turf loss area (sq ft)  ×  55.8 gal/sq.ft/yr\nSource: SNWA Xeriscape Conversion Study (2000)',
//     aboutPipeline
//   ),

//   ui.Label('Spatial units', {fontSize: '11px', fontWeight: 'bold', color: '#' + COLORS.black, margin: '4px 0 0 0'}),
//   ui.Label(
//     '2020 TIGER census tracts for Clark County, clipped to the urbanised study area. Tracts larger than 20 km² are excluded to focus on the built environment.',
//     aboutMuted
//   ),
...
//   // ── Limitations
//   ui.Label('Limitations', STYLES.subtitle),
//   ui.Label('▸  LST is land surface temperature, not air temperature — actual heat felt by residents may differ.', aboutMuted),
//   ui.Label('▸  Greenness loss could reflect xeriscape conversion, paving, construction, or drought — not exclusively turf removal.', aboutMuted),
//   ui.Label('▸  Water savings use a fixed conversion factor (55.8 gal/sq.ft/yr), not metered consumption data.', aboutMuted),
//   ui.Label('▸  FVC currently uses NDVI as a proxy. Spectral Mixture Analysis outputs will replace this in a future update.', aboutMuted),
...
//   ui.Label('References', STYLES.subtitle),
//   ui.Label(
//     '• Nevada AB 356 — Removal of non-functional turf (2021)\n' +
//     '• SNWA Xeriscape Conversion Study (2000)\n' +
//     '• SNWA Water Resource Plan 2026\n' +
//     '• U.S. Census Bureau TIGER/Line 2020 Tracts\n' +
//     '• Copernicus Sentinel-2 SR Harmonized\n' +
...
//   // update in v6 - use another function for reselect
//   updateHighlightLayer();
// }

// // update in v6
// // Safely looks for the highlight layer 
// // Removes it without touching the satellite imagery
// function updateHighlightLayer() {
//   // Find and remove ALL existing highlight and comparison layers
//   [leftMap, rightMap].forEach(function(m) {
//     var layers = m.layers();
//     var layersToRemove = [];
...
//   // Draw highlights for any saved compare tracts
...
//   // Draw the new highlight if a tract is currently selected
...
//     compareContent.widgets().reset([
//       makeSectionTitle('Compare areas'),
//       ui.Label('Click on the map to select your first tract for comparison.', {fontSize: '12px', color: '#' + COLORS.gray, padding: '12px 0'})
//     ]);
//     return;
//   }
...
//     compareContent.widgets().reset([
//       makeSectionTitle('Compare areas'),
//       ui.Label('● Tract ' + state.compareTracts[0].geoid, {fontSize: '13px', fontWeight: 'bold', color: '#' + state.compareTracts[0].color, padding: '4px 0'}),
//       resetCompareBtn,
//       ui.Label('Click on the map to select a second tract to compare against.', {fontSize: '12px', color: '#' + COLORS.gray, padding: '12px 0'})
//     ]);
//     return;
//   }
...
//   compareContent.widgets().reset([
//     makeSectionTitle('Compare areas'),
//     ui.Label('● Tract ' + tractA.geoid, {fontSize: '13px', fontWeight: 'bold', color: '#' + tractA.color, padding: '4px 0 2px 0'}),
//     ui.Label('● Tract ' + tractB.geoid, {fontSize: '13px', fontWeight: 'bold', color: '#' + tractB.color, padding: '0 0 8px 0'}),
//     resetCompareBtn,
//     ui.Label('Generating comparison charts...', {fontSize: '11px', color: '#' + COLORS.gray, fontStyle: 'italic', padding: '12px 0'})
//   ]);
...
//     // Replace the loading text with the actual charts
