// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v14
// change stats in 2 tracts comparison in the same month
// ═══════════════════════════════════════════════════════════════════════
...
function updateCompareView() {
  ...
  // Check if we are comparing across time or just a single month
  var isSameDate = (state.fromYear === state.toYear && state.fromMonth === state.toMonth);
  ...
  if (isSameDate) {
    // ─── SINGLE MONTH MODE (2 Boxes) ───
    var fvcImg = getFVCImage(state.fromYear, state.fromMonth);
    var lstImg = getLSTImage(state.fromYear, state.fromMonth);

    ee.Dictionary({
      fA: fvcImg.reduceRegion({reducer: ee.Reducer.mean(), geometry: tractA.feature.geometry(), scale: 10, bestEffort: true}).get('FVC'),
      fB: fvcImg.reduceRegion({reducer: ee.Reducer.mean(), geometry: tractB.feature.geometry(), scale: 10, bestEffort: true}).get('FVC'),
      lA: lstImg.reduceRegion({reducer: ee.Reducer.mean(), geometry: tractA.feature.geometry(), scale: 30, bestEffort: true}).get('LST'),
      lB: lstImg.reduceRegion({reducer: ee.Reducer.mean(), geometry: tractB.feature.geometry(), scale: 30, bestEffort: true}).get('LST')
    }).evaluate(function(v) {
      if (!v) return;
      var fA = v.fA || 0; var fB = v.fB || 0;
      var lA = v.lA || 0; var lB = v.lB || 0;

      // Tract B minus Tract A
      var fvcDiff = fB - fA;
      var lstDiff = lB - lA;
      ...
      var detailsText = ui.Label(
        'Raw values for ' + MONTHS[state.fromMonth - 1] + ' ' + state.fromYear + ':\n\n' +
        'Tract ' + tractA.geoid + '\n  FVC: ' + fA.toFixed(3) + '  |  Temp: ' + lA.toFixed(1) + '°C\n\n' +
        'Tract ' + tractB.geoid + '\n  FVC: ' + fB.toFixed(3) + '  |  Temp: ' + lB.toFixed(1) + '°C',
        {fontSize: '11px', color: COLORS.black, whiteSpace: 'pre-wrap', padding: '8px 0'}
      );
      ...
      compareContent.add(detailsText);
    });

  } else {
    // ─── COMPARE TIME MODE (Trend Charts) ───
