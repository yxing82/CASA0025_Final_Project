// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v4
// ═══════════════════════════════════════════════════════════════════════
...
// ─── RESTORED: Trend panel from original version ───
...
// ─── RESTORED: Nearby areas for context from original version ───
...
var compareContent = ui.Panel([
  ...
  ui.Label(
    'Add 2–3 tracts from Explore using "+ Add to comparison". Full comparison outputs can be added later.',
    {fontSize: '12px', color: COLORS.gray, whiteSpace: 'pre-wrap'}
  )
], null, STYLES.section);
...
function updateSidePanel(tract) {
  ...
  // ─── RESTORED Trend placeholder text ───
  trendPanel.widgets().reset([
    ui.Label('Trend over time', STYLES.subtitle),
    ui.Label(
      'Monthly FVC and LST charts will appear here once precomputed stats are available.',
...
function updateCompareView() {
  ...
  if (n < 2) {
    compareContent.widgets().reset([
      makeSectionTitle('Compare areas'),
      ui.Label(
        n + '/2 tracts selected. Go to Explore, click a tract, and use "+ Add to comparison".',
        {fontSize: '12px', color: COLORS.gray, whiteSpace: 'pre-wrap'}
      )
    ]);
    return;
  ...
  widgets.push(ui.Label(
    'Reserved for later: side-by-side trend charts, normalized metrics, and ranked summary tables from precomputed tract statistics.',
    {fontSize: '11px', color: COLORS.gray, whiteSpace: 'pre-wrap', padding: '10px 0 0 0'}
  ));
