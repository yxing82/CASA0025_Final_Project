// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v31
// readjust the baseline comparison
// ═══════════════════════════════════════════════════════════════════════
...
//     // FIX 1: Updated to check state.dataType instead of state.timeMode
...
//       // FIX 2: Cleared the trend charts for actual data
//       trendPanel.widgets().reset([]); 
...
//       // FIX 3: Cleared the trend charts for actual data
//       trendPanel.widgets().reset([]); 
//     }
...
function updateSidePanel(tract) {
  ...
  ee.Dictionary({
    ...
    if (state.dataType === 'adjusted') {
      ...
      cardTemp.delta.setValue('compared to same month in 2019'); 
  ...
  // only calculate distribution charts if we are using actual data
  if (state.dataType !== 'adjusted') {
    updateDistributionCharts(tractGeom);
  }
}
...
// var explorePanel = ui.Panel([promptPanel, tractHeaderPanel, statsPanel, chartsPanel, trendPanel]);
