// ═══════════════════════════════════════════════════════════════════════
// LAS VEGAS VALLEY TURF TRACKER — UI FRAMEWORK v33
...
function updateSidePanel(tract) {
  ...
  if (state.dataType === 'adjusted') {
    ...
    ee.Dictionary({
      ...
      // Re-arrange cards line by line for adjusted data
...
function updateCompareView() {
  ...
  if (state.dataType === 'adjusted') {
      ...
      ee.Dictionary({
        ...
        var makeAdjustedCard = function(tractObj, metricLabel, val, valFormatter, isReverseColor) {
            var valSafe = val || 0;
            
            // Highlighted Tract ID text combined with metric label
            ...
            ], ui.Panel.Layout.flow('horizontal'), {margin: '0 0 6px 0', backgroundColor: 'transparent'});

            // Stat Value
            var valLabel = ui.Label(valFormatter(valSafe), STYLES.statValue);
            
            // Subtext coloring based on metric type 
            var deltaColor = valSafe < 0 ? COLORS.red : COLORS.green;
            if (isReverseColor) {
                deltaColor = valSafe > 0 ? COLORS.red : COLORS.green;
            }
            
            // Tract Profile style sub-label
            var subtextLabel = ui.Label(monthLabel + ' vs same month in 2019', {fontSize: '11px', color: '#' + deltaColor, margin: '4px 0 0 0'});
            
            return ui.Panel([titleRow, valLabel, subtextLabel], null, STYLES.card);
        };

        var fvcCardA = makeAdjustedCard(tractA, 'Greenness', v.fA, function(x){ return (x * 100).toFixed(1) + '%'; }, false);
        var fvcCardB = makeAdjustedCard(tractB, 'Greenness', v.fB, function(x){ return (x * 100).toFixed(1) + '%'; }, false);
        var lstCardA = makeAdjustedCard(tractA, 'Temperature', v.lA, function(x){ return (x > 0 ? '+' : '') + x.toFixed(1) + '°C'; }, true);
        var lstCardB = makeAdjustedCard(tractB, 'Temperature', v.lB, function(x){ return (x > 0 ? '+' : '') + x.toFixed(1) + '°C'; }, true);
        var etCardA  = makeAdjustedCard(tractA, 'Water', v.eA, function(x){ return (x > 0 ? '+' : '') + x.toFixed(1) + ' mm'; }, true);
        var etCardB  = makeAdjustedCard(tractB, 'Water', v.eB, function(x){ return (x > 0 ? '+' : '') + x.toFixed(1) + ' mm'; }, true);
